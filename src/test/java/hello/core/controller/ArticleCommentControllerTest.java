...
import hello.core.util.FormDataEncoder;
...
@Import({TestSecurityConfig.class, FormDataEncoder.class}) // Custom Annotation을 제작해서 사용할때 다양한 매타 관계를 붙혀서 작업.
@WebMvcTest(ArticleCommentController.class) // Controller를 테스트하기 위한 어노테이션
class ArticleCommentControllerTest {

    private final MockMvc mvc;
    private final FormDataEncoder formDataEncoder;

    // 실제 Bean이 아닌 Mock 객체를 ApplicationContext에 주입
    @MockBean private ArticleCommentService articleCommentService;

    // MockBean 이 생성자 주입을 지원하지 않기 때문에 Autowirde 필수
    ...
    @WithUserDetails(value = "leeseungseok", setupBefore = TestExecutionEvent.TEST_EXECUTION)
